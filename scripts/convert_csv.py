import os
import pandas as pd
import re
from unidecode import unidecode
import config


def find_all_files():
    upload_files = os.listdir('upload')

    for file in upload_files:
        if file.endswith('.xlsx') or file.endswith('.xls'):
            abas = config.abas.get(file)

            if abas:
                for aba in abas:
                    convert_csv(file, aba)
            else:
                convert_csv(file)


def convert_csv(file, aba=None):
    # ✅ Leitura SEM conversão automática (ESSENCIAL)
    if aba is None:
        read_file = pd.read_excel(f'upload/{file}', dtype=str)
    else:
        read_file = pd.read_excel(f'upload/{file}', sheet_name=aba, dtype=str)

    # ✅ Limpeza básica
    read_file = read_file.replace('\n', ' ', regex=True)
    read_file = read_file.replace('\r', '', regex=True)

    # ✅ Padroniza nomes das colunas
    new_columns = []
    for column in read_file.columns:
        new_columns.append(snake_small_case(column))

    read_file.columns = new_columns

    # ✅ Corrige valores (mantém vírgula como decimal)
    if 'valor' in read_file.columns:
        read_file['valor'] = read_file['valor'].str.replace('.', ',', regex=False)

    # ✅ Nome do arquivo
    csv_name = file.split('.xls')[0]
    csv_name = csv_name if aba is None else f'{aba}'

    # ✅ Salva CSV no padrão correto
    read_file.to_csv(
        f'dataset/data/{csv_name}.csv',
        index=False,
        header=True,
        sep=';',
        encoding='utf-8-sig',
        na_rep=""
    )


def snake_small_case(column):
    column_lower = column.lower()
    column_unidecode = unidecode(column_lower)
    column_alphanumeric = re.sub('[^A-Za-z0-9]+', ' ', column_unidecode)
    column_split = column_alphanumeric.split(' ')
    column_empty = [x for x in column_split if x != '']
    column = '_'.join(column_empty)
    return column


if __name__ == '__main__':
    find_all_files()
